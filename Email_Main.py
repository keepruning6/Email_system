...
class EmailSystem:
    ...
    def destory_windows(self):
        self.remain_windows.destroy()                #销毁当前窗口
    ...
    def send_email(self):
        ...
        # 加入文本消息
        msg = MIMEMultipart()
        msg['From'] = 'keshe_EMAIL_SYSTEM <' + self.user + '>'      #发件人信息
        msg['To'] = self.receiver                                 #收件人信息
        msg['Subject'] = self.subject                             #邮件主题
        #_send_text = Encryption().Encry_text(str(self._send_text))  # AES加密后的密文
        _send_text = MIMEText(self._send_text, 'plain', 'utf-8')         #设置纯文本格式
        msg.attach(_send_text)                                      #邮件正文内容

        # 加密文件
        #cipher_path = []
        #for path in self.path:
        #    cipher_path.append(Encryption().Encry_file(path))  # 加密图片文件  此处调用Encyption.py模块，会变成一个bin文件后缀
        #self.path[:] = []
        #for path in cipher_path:
        #    self.path.append(path)  # 将加密后的附件路径添加至发送列表 将添加了bin文件后缀的重新添加到路径中
        ...
        if len(self.path) != 0:
            try:
                for file_path in self.path:
                    ...
                    file_part.add_header('Content-Disposition', 'attachment', filename=file_path)#为附件命名
                    msg.attach(file_part)                                                        #添加附件
                print "ADD SUCCESS"
            except Exception as e:
                print "ADD FILE ERROR ->", str(e)

        self.server.set_debuglevel(1)                     #发送调试消息到服务器打印出和SMTP服务器交互的所有信息
        self.server = smtplib.SMTP(self.Smtp_Server, 25)  # 与邮件服务器重新建立连接
        self.server.login(self.user, self.passwd)  # 重新登入
        self.remain_windows = Tk()                 #调用窗口对象
        self.remain_windows.title("提示消息")
        try:
            self.server.sendmail(self.user, self.receiver, msg.as_string())  #发送发信人，收信人，正文
            self.server.quit()                                               #发送结束，退出保存
            remain_lable = Frame(self.remain_windows)                        #调用一个纯容器
            Label(remain_lable, text="Send Success!",bg="gray",width="20",height="2").pack(fill="y", expand="yes") #调用一个标签，内容为发送成功
            remain_lable.pack()                                                    #显示这个标签
            self.remain_windows.mainloop()                                         #完成窗口的循环调用
        except Exception as e:
            print "Send Error - >", str(e)                                         #发送异常信息
            remain_lable = Frame(self.remain_windows)                              #在窗口处显示发送失败
            Label(remain_lable, text="Send Failed!").pack(fill="y", expand="yes")
            remain_lable.pack()
            self.remain_windows.mainloop()

        exit_button = Frame(self.remain_windows)                                 #调用一个纯容器
        Button(exit_button, text="Confirm", command=self.destory_windows).pack() #调用一个确认按钮，并显示确认字样，显示之后摧毁窗口
        exit_button.pack()                                                       #显示这个按钮

    def add_file(self):                  #增加文件函数
        try:
    ...
    def send_windows(self):
        ...
        self.main_windows.resizable(width=True, height=True)  #高宽可变
        ...
        self.encryption['value'] = ("不加密",'AES', "base64")  # 默认使用AES加密，还有一个激就是默认的base64
    ...
    def check(self):
        ...
        self.check.geometry('400x200')
        self.check.resizable(width=True, height=True)
        remain_info_text = Frame(self.check)                       #获取框架
        try:
            self.user = self.param_1.get()                         #获取用户名
            self.passwd = self.param_2.get()                       #获取密码
        ...
        try:
            ...
            self.server.quit()
            print "Log In success"          #在终端打印登录成功
            remain_info_text = Frame(self.check)
            Label(remain_info_text, text="Log In Success").pack(fill="y", expand="yes")  #在界面打印登录成功
            remain_info_text.pack()
            confirm_button_frame = Frame(self.check)
            Button(confirm_button_frame, text="Confirm", command=self.index_windows,width=20,bg="gray",font=("Arial", 15)).pack()  # 登陆成功后点击确认进入主要界面
    ...
    def main(self):
        ...
        self.EMail_UI.title("Email System - 课设")
        self.EMail_UI.geometry('500x250')
        ...
        # 提示文字
        Label(self.EMail_UI, text="用户名:").place(x=50, y=20)
        Label(self.EMail_UI, text="密码:").place(x=50, y=60)

        # 输入信息
        Entry(self.EMail_UI, textvariable=self.param_1).place(x=150, y=20)
        Entry(self.EMail_UI, textvariable=self.param_2, show='*').place(x=150, y=60)  # 加密输入

        # 按钮
        Button(self.EMail_UI, text='登陆', command=self.check, bg='white', width=10, height=1).place(x=40, y=120)
        Button(self.EMail_UI, text='退出', command=self.EMail_UI.destroy, bg='white', width=10, height=1).place(x=200, y=120)
